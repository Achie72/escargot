...
static Value builtinPromiseFinally(ExecutionState& state, Value thisValue, size_t argc, Value* argv, bool isNewExpression)
{
    ...
    if (onFinally.isPointerValue() && onFinally.asPointerValue()->isCallable()) {
        Value valueArgs[] = {C, onFinally};
        thenFinally = new ExtendedNativeFunctionObject(state, NativeFunctionInfo(AtomicString(state, String::emptyString), promiseThenFinally, 1, NativeFunctionInfo::Strict), valueArgs);

        catchFinally = new ExtendedNativeFunctionObject(state, NativeFunctionInfo(AtomicString(state, String::emptyString), promiseCatchFinally, 1, NativeFunctionInfo::Strict), valueArgs);
    }
    Value then = thisObject->get(state, strings->then).value(state, thisObject);
    return Object::call(state, then, thisObject, 2, arguments);
}
